...
void registerPub(rclcpp::Node::SharedPtr n)
{
    ...
    static bool vins_result_file_reset = false;
    if (!vins_result_file_reset)
    {
        ensure_parent_dir(VINS_RESULT_PATH);
        std::ofstream fout_reset(VINS_RESULT_PATH, std::ios::out | std::ios::trunc);
        fout_reset.close();
        vins_result_file_reset = true;
        ROS_INFO("reset VINS result file: %s", VINS_RESULT_PATH.c_str());
    }
