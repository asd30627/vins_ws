...
# ============================================================
# KAIST FOG reliability future-drift pipeline
# 1. delete old admission labels/datasets
# 2. build future-drift labels for each sequence
# 3. combine all sequences
...
COMBINED_DIR="${RESULT_ROOT}/_combined_future_reliability"

SEQ_LEN=16
HORIZON_STEPS=50

TRANS_THRESHOLD_M=0.50
ROT_THRESHOLD_DEG=5.0

# 如果 GT timestamp 對不上，之後可以改成 0.10 或 0.20
MAX_GT_TIME_GAP=0.05

# 正式論文建議 0：沒有 GT 就跳過，不要混 proxy label
ALLOW_PROXY=0
...
for seq in "${SEQS[@]}"; do
    ...
    if [ -d "${ADM}" ]; then
        echo "[CLEAN] ${seq}: remove old labels/datasets"
        rm -rf "${ADM}/labels"
        rm -rf "${ADM}/datasets"
    fi
done
...
echo
echo "========== [2] Build labels for each sequence =========="

for seq in "${SEQS[@]}"; do
    ADM="${RESULT_ROOT}/${seq}/admission"
    FEATURE_CSV="${ADM}/features/reliability_features_vins.csv"
    LABEL_DIR="${ADM}/labels"
    ...
    if [ -n "${GT_CSV}" ]; then
        ...
        python3 "${PIPE_DIR}/build_reliability_labels.py" \
            ...
            --trans_scale_m "${TRANS_THRESHOLD_M}" \
            --rot_scale_deg "${ROT_THRESHOLD_DEG}" \
...
echo
echo "========== [3] Combine all feature/label csv =========="
...
for seq_dir in seq_dirs:
    ...
    feature_csv = adm / "features" / "reliability_features_vins.csv"
    label_csv = adm / "labels" / "reliability_labels.csv"
